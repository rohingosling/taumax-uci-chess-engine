...
impl SearchLimits {
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Predicate Accessor: allows_root_move
    //
    // Description:
    //
    //   Return whether a UCI root move is allowed by the searchmoves filter.
    //
    //------------------------------------------------------------------------------------------------------------------

    pub fn allows_root_move(&self, move_text: &str) -> bool {

        // With no searchmoves filter, every legal root move is allowed.

        self.search_moves.is_empty()
            || self
                .search_moves
                .iter()
                .any(|allowed_move| allowed_move == move_text)
    }
    ...
    pub fn parse_from_tokens(tokens: &[&str]) -> Self {
        ...
        // Start from default limits and fill only the options present in the command.
        ...
        while token_index < tokens.len() {
            match tokens[token_index] {
                ...
                _ => {
                    ...
                    // Unknown tokens are skipped so newer GUI options do not break the engine.
...
mod tests {
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Function: searchmoves_filter_allows_only_named_root_moves
    //
    // Description:
    //
    //   Verify the root-move filter honors GUI-supplied searchmoves.
    //
    //------------------------------------------------------------------------------------------------------------------

    #[test]
    fn searchmoves_filter_allows_only_named_root_moves() {

        // Build a limits value with two allowed root moves.

        let limits = SearchLimits::parse_from_tokens(&["searchmoves", "e2e4", "d2d4"]);

        // Verify named moves pass and an unnamed legal-looking move is rejected.

        assert!(limits.allows_root_move("e2e4"));
        assert!(limits.allows_root_move("d2d4"));
        assert!(!limits.allows_root_move("g1f3"));
    }
}
