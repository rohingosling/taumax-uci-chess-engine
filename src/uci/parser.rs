...
// Description:
//
//   Permissive parser for the subset of UCI used by this engine.
//
//----------------------------------------------------------------------------------------------------------------------
...
pub fn parse_uci_command(text: &str) -> UciCommand {
    ...
    // Trim line endings and surrounding spaces before tokenizing the command.

    let trimmed_text = text.trim();

    if trimmed_text.is_empty() {

        // Empty lines are harmless and are treated like unknown commands.
...
fn parse_startpos_command(tokens: &[&str]) -> Option<PositionCommand> {
    ...
    if tokens[0] != "moves" {
        ...
        // Any token other than "moves" after startpos is outside the supported grammar.
...
fn parse_fen_command(tokens: &[&str]) -> Option<PositionCommand> {
    if tokens.is_empty() {
        ...
        // "position fen" without the actual FEN fields is malformed.
    ...
    if fen_token_count == 0 {
        ...
        // A moves marker before any FEN text leaves no base position to parse.
...
fn parse_setoption_command(tokens: &[&str]) -> UciCommand {
    ...
    // Preserve the parsed shape so the session layer can decide whether the option is known.
...
mod tests {
    ...
    fn parse_uci_handshake() {
        ...
        // The plain "uci" line starts the identity and option-advertisement handshake.
    ...
    fn parse_startpos_with_moves() {
        ...
        // The parser should keep every move token after the moves marker.
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Function: parse_setoption_with_spaced_name
    //
    // Description:
    //
    //   Verify setoption commands preserve user-facing option names that contain spaces.
    //
    //------------------------------------------------------------------------------------------------------------------
    ...
    fn parse_setoption_with_spaced_name() {
        ...
        // Option names may contain spaces, so the parser must join all name tokens before value.

        assert_eq!(
            parse_uci_command("setoption name Diagnostics Trace value true"),
            UciCommand::SetOption {
                name: "Diagnostics Trace".to_string(),
