...
fn has_bestmove_line(output: &str) -> bool {
    ...
    // Without a fixed Random Seed, the exact move may vary, so these tests assert the protocol shape.
...
fn startpos_moves_then_go_returns_bestmove() {
    ...
    // Start from the normal chess opening position, replay two moves, then ask for a move.

    let output = run_engine_script("position startpos moves e2e4 e7e5\ngo movetime 1\nquit\n");

    // The exact move can vary, but the protocol response must include bestmove.
...
fn fen_then_go_returns_bestmove() {
    ...
    // Start from an explicit FEN position rather than the shorthand startpos command.

    let output = run_engine_script(
        "position fen rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1\ngo depth 1\nquit\n",
    );

    // The FEN setup should be accepted and followed by a bestmove response.
